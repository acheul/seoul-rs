...
pub(super) fn impl_reflica_macro(ast: &DeriveInput) -> Result<TokenStream> {
    ...
    // parsing generics (add 'rfc lifetime for ref type)
    let (impl_generics, ty_generics, where_clause) = ast.generics.split_for_impl();

    let mut _generics = ast.generics.clone();
    let lt = syn::Lifetime::new("'rfc", Span::call_site());
    let ltp = syn::LifetimeParam::new(lt);
    _generics.params.push(syn::GenericParam::from(ltp));

    let (ref_impl_generics, ref_ty_generics, ref_where_clause) = _generics.split_for_impl();
...
#ref_fields
// Into
impl #ref_impl_generics Into<#ref_name #ref_ty_generics> for &'rfc #name #ty_generics #ref_where_clause {
...
impl #ref_impl_generics Into<#ref_name #ref_ty_generics> for &'rfc #name #ty_generics #ref_where_clause {
    fn into(self) -> #ref_name #ref_ty_generics {
        ...
        Data::Union(_) => return Err(Error::new(ast.span(), "Not for Union data type")),
    }
...
pub fn get_ref_fields(fields: &Fields) -> (bool, TokenStream, TokenStream) {
    ...
    match fields {
        Fields::Named(fields) => {
            ...
            for (i, field) in fields.named.iter().enumerate() {
                ...
                quote.extend(if i + 1 == len {
                    quote! { #vis #name: &'rfc #ty }
                } else {
                    quote! { #vis #name: &'rfc #ty, }
                });

                quote2.extend(if i + 1 == len {
                    quote! { #name }
                } else {
                    quote! { #name, }
                });
        ...
        Fields::Unit => (false, quote, quote2),
    }
}
